...
# يُنفَّذ بعد pre-ren.sh مباشرةً — لا يكرر ما فيه
# يتولى: تنظيف Docker + تشغيل الشبكة + نشر Chaincode + Caliper
# ============================================================
...
# ─── 4: تصدير PATH (يضمن وصول peer/configtxgen) ──────────────────────────────
export PATH="${ROOT_DIR}/bin:$PATH"
export FABRIC_CFG_PATH="${ROOT_DIR}/config/"

# ─── 5: تشغيل شبكة Fabric ────────────────────────────────────────────────────
echo ""
echo -e "${GREEN}=== تشغيل شبكة Fabric ===${NC}"
...
echo "انتظار 30 ثانية لاستقرار CouchDB والـ Peers..."
sleep 30
cd "${ROOT_DIR}"
...
# ─── 7: الكشف الديناميكي عن المفاتيح والشهادات ────────────────────────────────
echo ""
echo "=== كشف المفاتيح الخاصة والشهادات ==="
...
# ─── 9: إنشاء networkConfig.yaml ─────────────────────────────────────────────
echo ""
echo "=== إنشاء networkConfig.yaml ==="
mkdir -p networks
...
# ─── 10: إنشاء connection profiles كاملة (مع orderer) ────────────────────────
echo "=== إنشاء connection profiles ==="
...
# ─── 11: تشغيل Caliper Benchmark ──────────────────────────────────────────────
echo ""
echo -e "${GREEN}=== تشغيل Caliper Benchmark ===${NC}"
