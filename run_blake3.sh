...
log "Step 1: Rebuilding Fabric Network (Tuned for BLAKE3)..."
cd "${PROJECT_ROOT}/test-network"
./network.sh down
docker volume prune -f 2>/dev/null || true
docker network prune -f 2>/dev/null || true  # ✅ إضافة: تنظيف الشبكات المتبقية
./network.sh up createChannel -ca -s couchdb
sleep 20  # ✅ إصلاح: زيادة من 15 إلى 20 ثانية لضمان جاهزية كل الـ containers
...
# ✅ إصلاح: حذف peer lifecycle chaincode package المكررة
# deployCC تقوم بالتحزيم والنشر تلقائياً — لا داعي لأمر منفصل
...
# ✅ إصلاح: حذف blake3@^0.3.1 — هذا الإصدار غير موجود في npm
# نثبّت blake3 بدون تحديد إصدار ليأخذ أحدث نسخة متاحة
...
CERT_FILE1="${CERT_DIR1}/cert.pem"  # ✅ إصلاح: cert.pem ثابت دائماً بعد network.sh up -ca

log "Detecting Org1 keys and certificates dynamically..."
if [ -z "$PVT_KEY1" ]; then
    error "Org1 private key not found — is the network running?"
    exit 1  # ✅ إصلاح: الخروج بدل المتابعة بـ PLACEHOLDER
fi
...
CERT_FILE2="${CERT_DIR2}/cert.pem"  # ✅ إصلاح: cert.pem ثابت دائماً
...
# ✅ إصلاح: إنشاء connection profiles قبل networkConfig
mkdir -p networks

# إنشاء connection-org1.yaml
...
# ✅ إصلاح: حذف fabric-gateway المتعارضة فقط إذا كانت موجودة
