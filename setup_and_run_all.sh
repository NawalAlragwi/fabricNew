...
#    - Run with: bash setup_and_run_all.sh (not ./setup_and_run_all.sh on macOS/zsh)
#
# ============================================================================
...
setup_fabric_network() {
    ...
    log "Waiting for network stabilization (orderer ready)..."
    until docker ps | grep -q orderer; do
        log "  Waiting for orderer container..."
        sleep 3
    done
    ...
    #
    # FIX-4: GOWORK=off prevents go.work conflicts if they exist.
    # ──────────────────────────────────────────────────────────────────────────
    ...
    # Verify smartcontract file exists
    if [ ! -f "${ROOT_DIR}/chaincode-bcms/hybrid-batch/smartcontract_hybrid.go" ]; then
        error "❌ smartcontract_hybrid.go not found"
        exit 1
    fi
    log "✓ smartcontract_hybrid.go found"

    info "📦 Deploying BCMS Hybrid-Batch chaincode (SHA-256 + BLAKE3 + Batch)..."
    
    # Clean up any old package files that might interfere
    log "Cleaning up old package files..."
    rm -f "${ROOT_DIR}/test-network/basic.tar.gz" "${ROOT_DIR}/test-network/log.txt" 2>/dev/null || true
    
    # Set Go environment variables for successful packaging
    ...
    # Run chaincode deployment from test-network directory
    ./network.sh deployCC \
        -ccn basic \
        ...
        error "   Recent errors:"
        tail -50 "$LOG_FILE" | grep -i "error\|failed" | tail -10
        exit 1
    }
...
# ─── reporting_pipeline: aggregate → four-scenario → individual reports ───────
reporting_pipeline() {
    local context="${1:-all}"
    step "Reporting Pipeline — ${context}"

    # Step A: run parse_caliper_report.py for each scenario that has a real report.html
    for key in scenario_1_sha256 scenario_2_blake3 scenario_3_merged scenario_4_batching; do
        ...
        local report_html="caliper-workspace/report.html"
        if [ -f "$report_html" ]; then
            log "  Parsing real Caliper report for scenario ${n}..."
            python3 scripts/parse_caliper_report.py \
                --report "$report_html" \
                --scenario "$n" \
    ...
    # Step B: aggregate all scenario data → comparison_data.json
    log "  Step B: aggregate_results.py → results/final_comparison/comparison_data.json"
    python3 aggregate_results.py 2>&1 | tee -a "$LOG_FILE" || warn "aggregate_results.py failed"

    # Step C: master four-scenario comparison dashboard
    log "  Step C: generate_four_scenario_report.py → results/final_comparison/four_scenario_report.html"
    python3 generate_four_scenario_report.py 2>&1 | tee -a "$LOG_FILE" || warn "generate_four_scenario_report.py failed"

    # Step D: individual per-scenario reports (reads from comparison_data.json for multipliers)
    log "  Step D: generate_individual_reports.py → results/scenario_*/report_*.html"
    ...
    log "    results/final_comparison/comparison_data.json    (aggregated data)"
    log "    results/final_comparison/four_scenario_report.html (master comparison)"
    log "    results/four_scenario_report.html                 (root-level copy)"
    ...
    log "    results/final_comparison/comparison_report.md"
}
...
main() {
    ...
    if [ "$REPORT_ONLY" = "true" ]; then
        info "REPORT_ONLY mode: regenerating all reports from existing data"
        mkdir -p results
        generate_tamarin_html_report
    ...
    # Step 8: Full reporting pipeline (parse → aggregate → four-scenario → individual)
